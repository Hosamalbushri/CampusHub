...
>
    <div class="w-full max-w-none px-4 py-8 sm:px-6 lg:px-[60px] lg:py-10">
        ...
        <nav
            class="mb-6 border-b border-slate-200 pb-4"
        ...
        >
            <ol class="flex flex-wrap items-center gap-x-2 gap-y-1 text-sm text-slate-600">
                ...
                <li
                    class="text-slate-300 select-none"
                    aria-hidden="true"
                >
                    /
                </li>
                ...
                <li
                    class="min-w-0 max-w-full font-semibold text-slate-900"
                    aria-current="page"
                >
        ...
        <article class="w-full max-w-none overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            @if ($event->image)
            ...
            <div class="w-full p-6 sm:p-8 lg:p-10">
                ...
                <div class="mt-8">
                    @if ($isSubscribedToCurrentEvent)
                        <span
                            class="inline-flex min-h-[44px] w-full cursor-default items-center justify-center rounded-xl border border-emerald-200 bg-emerald-50 px-6 py-2.5 text-sm font-semibold text-emerald-800 sm:w-auto sm:min-w-[220px]"
                            role="status"
                        >
                ...
                >
                    @if ($event->description)
                        <div class="mt-8 max-w-none space-y-4 leading-relaxed text-slate-700 [&_a]:font-medium [&_a]:text-[color:var(--shop-accent)] [&_h1]:text-2xl [&_h1]:font-semibold [&_h2]:mt-4 [&_h2]:text-xl [&_h2]:font-semibold [&_ul]:list-disc [&_ul]:ps-6">
                            {!! $event->description !!}
                        </div>
                    @endif
                    ...
                    @if (($event->fields ?? collect())->isNotEmpty())
                        <section class="mt-10 border-t border-slate-100 pt-8">
                            <h2 class="text-lg font-semibold text-slate-900">{{ __('shop::app.events.show.details') }}</h2>
