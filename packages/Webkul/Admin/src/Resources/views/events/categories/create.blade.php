<x-admin::layouts>
    <!-- Page Title -->
    <x-slot:title>
        @lang('admin::app.event-categories.create.title')
    </x-slot>

    <!-- Create Form -->
    <x-admin::form
        :action="route('admin.events.categories.store')"
        method="POST"
    >
        <div class="flex flex-col gap-4">
            ...
            <div class="flex items-center justify-between rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300">
                <div class="flex flex-col gap-2">
                    <x-admin::breadcrumbs name="categories.create" />
                ...
                <div class="flex items-center gap-x-2.5">
                    <div class="flex items-center gap-x-2.5">
                        ...
                        >
                            @lang('admin::app.event-categories.create.save-btn')
                        </button>
                    </div>
                </div>
            </div>
                        ...
                        <x-admin::form.control-group>
                            ...
                            >
                                ...
                                @foreach ($parentCategories as $p)
                                    <option value="{{ $p->id }}" @selected(old('parent_id') !== null && (int) old('parent_id') === (int) $p->id)>
                                        {{ $p->name }}
                                    </option>
                                @endforeach
                            </x-admin::form.control-group.control>
